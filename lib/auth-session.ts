...
type SessionPayload = { userId: string; iat: number; exp: number };
...
function encodeToken(payload: SessionPayload, secret: string) {
...
function decodeToken(token: string, secret: string): SessionPayload | null {
...
export async function setSessionCookie(userId: string) {
  ...
  const token = encodeToken({ userId, iat: now, exp: now + SESSION_TTL_SECONDS }, secret);
