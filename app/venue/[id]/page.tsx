        ...
        <section className="space-y-5">
          <header className="glass rounded-3xl p-5">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <Badge>{venue.crowdLabel ?? "Nightlife"}</Badge>
              <Badge className="bg-fuchsia-500/20">
                Buzz {venue.buzzScore ?? 0}
              </Badge>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card className="space-y-3 p-4">
              ...
              <p className="text-sm text-zinc-300">
                ...
                {venue.priceLevel ? "$".repeat(venue.priceLevel) : "-"}
              </p>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card className="space-y-2 p-4">
              ...
              <p className="text-sm text-zinc-300">
                Compared to usual: {venue.comparedToUsual ?? "Unknown"}
              </p>
              <p className="text-sm text-zinc-300">
                Confidence: {venue.confidence ?? "Unknown"}
              </p>
              <p className="text-sm text-zinc-300">
                Live reports in window: {venue.liveReportsCount ?? 0}
              </p>
            </Card>
            ...
            <Card className="space-y-2 p-4">
              ...
              <p className="text-sm text-zinc-300">
                Dress code: {venue.dressCode ?? "No verified dress code yet."}
              </p>
              <p className="text-sm text-zinc-300">
                Cover:{" "}
                {venue.coverTonight
                  ? `$${venue.coverTonight}`
                  : "No verified cover info yet."}
              </p>
            </Card>
          </div>
          ...
          <Card className="space-y-2 p-4">
            ...
            {!venue.liveReports?.length && (
              <p className="text-sm text-zinc-300">
                Not enough live data yet — be the first to report.
              </p>
            )}

            {venue.liveReports?.map((report: any) => (
              <p key={report.id} className="text-sm text-zinc-300">
                {report.crowdLabel} · {report.lineStatus} line ·{" "}
                {report.vibeNotes ?? "No note"}
              </p>
            ))}
          </Card>
        </section>
