...
export default async function VenuePage({ params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return (
      <AppShell title={venue.name} locationLabel="Central Jersey">
        <section className="space-y-5">
          <header className="glass rounded-3xl p-5">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <Badge className="gap-1">
                <Clock3 size={12} />
                {venue.isOpenNow === null ? "Hours unavailable" : venue.isOpenNow ? "Open now" : "Closed"}
              </Badge>
            </div>
          </header>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card className="space-y-3 p-4">
              ...
              <p className="text-sm text-zinc-300">
                {venue.totalReviews} reviews · {venue.priceLevel ? "$".repeat(venue.priceLevel) : "-"}
              </p>
