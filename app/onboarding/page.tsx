  ...
  useEffect(() => {
    async function loadSession() {
      try {
        ...
        const profile = user?.socialProfile ?? user?.profile;
        ...
        setPreferredCity(profile?.preferredCity ?? profile?.cityLine ?? "");
        setAgeRange(profile?.ageRange ?? "");

        if (Array.isArray(profile?.preferredNightlifeTypes)) {
          setPreferredNightlifeTypes(profile.preferredNightlifeTypes);
        }
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-black px-4 py-8 text-white sm:px-6 lg:py-12">
      ...
      <section className="relative mx-auto w-full max-w-3xl rounded-3xl border border-white/15 bg-gradient-to-b from-zinc-900/90 to-black/90 p-5 shadow-2xl backdrop-blur sm:p-8">
        ...
        {status === "loading" && <p className="mt-6 text-sm text-zinc-400">Loading your account…</p>}
        ...
        {status !== "loading" && (
          <form onSubmit={onSubmit} className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              <label htmlFor="ageRange" className="mb-1 block text-sm text-zinc-200">Age range</label>
              <input id="ageRange" className="w-full rounded-xl border border-white/15 bg-black/70 px-3 py-2 text-white outline-none transition focus:border-fuchsia-300 focus:ring-2 focus:ring-fuchsia-300/40" value={ageRange} onChange={(e) => setAgeRange(e.target.value)} placeholder="18-24" />
            </div>
                ...
                {NIGHTLIFE_OPTIONS.map((option) => {
                  ...
                  return (
                    <button
                      key={option}
                      type="button"
                      className={`rounded-full border px-3 py-1.5 text-sm capitalize transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70 ${
                        active
                          ? "border-cyan-300/70 bg-gradient-to-r from-cyan-400/25 to-fuchsia-400/25 text-cyan-100"
                          : "border-white/20 text-zinc-200 hover:border-fuchsia-300/60 hover:text-white"
                      }`}
                      onClick={() =>
            ...
            <div className="mt-2 sm:col-span-2">
              <button type="submit" disabled={status === "saving"} className="w-full rounded-2xl bg-gradient-to-r from-cyan-300 to-fuchsia-300 px-4 py-3 text-sm font-semibold text-black transition hover:brightness-110 active:scale-[0.99] disabled:opacity-70">
